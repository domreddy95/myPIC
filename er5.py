    ...
    for k in range (0,T):
        ...
        for i in range (1,501):
            if i == 0:
                ...
                valr = (np.real(tc[i]))/i/i
                vali = (np.imag(tc[i]))/i/i
                #print(valr,vali,coeff[i])
            avg_p[k] += valr**2 + vali**2
    plt.figure(12)
    t = np.linspace(0,tf,num=(len(avg_p)))
    plt.plot(t,avg_p)
...
def main(xl,xr,vl,vr):
    for j in range (0,T):
        ...
        total_charge[j] = np.sum(Rho)
        total_Elec[j] = np.sum(elec**2)
